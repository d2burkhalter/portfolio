...
def print_rdd(rdd, logfile):
  ...
  results = rdd.collect() #returns all the elements of rdd as an array
  counter = 0
  for result in results: #logs up to 30 elements of the rdd
    counter = counter + 1
    f.write(str(result) + "\n")
    if counter > 30:
      break
...
init()
lines = sc.textFile(ratings_file) #create a text file RDD from the movielens data
#movie id and rating in 1d array
rdd = lines.map(parse_line) # movie_id, rating
#print_rdd(rdd, "movie_rating_pairs")
#adds a dimension with value of 1 to movie rating element
...
#sums all of the ratings and number of ratings for each film
rdd_totals = rdd_pair.reduceByKey(add_ratings_by_movie) # movie_id (total rating, total occurrences)
#print_rdd(rdd_totals, "movie_total_rating_occurrences")
...
def avg_ratings_by_movie(rating_total_occur):
  ...
  return avg_rating
#takes total number of ratings and sum of ratings to return the average rating
...
#makes the RDD persist for future use
rdd_avgs.cache()
...
# lookup imdb id
links_lines = sc.textFile(links_file) #create an RDD from the movielens links file
#finds the imdb title_id for the movie_id number
rdd_links = links_lines.map(parse_links_line) # movie_id, imdb_id
#print_rdd(rdd_links, "rdd_links")

#combines the movie_id and movie ratings
...
# add imdb_id prefix ()
#combines the movielens rating with imdb title_id
...
def save_rating_to_db(list_of_tuples):
  ...
  for tupl in list_of_tuples:
    ...
    try:
        cur.execute(update_stmt, (avg_rating, imdb_id_str))#use prepared statement because escape characters might end sql statement early 
    except Exception as e:
        print "Error in save_rating_to_db: ", e.message

#connects to the rds database and updates movielens_rating for each movie in rdd
rdd_ratings_by_imdb.foreachPartition(save_rating_to_db)#for each partition calls the db so only one connection per executor instead of one connection per call
